//==========================================================
//==========================================================
$.jribbble.getShotsByPlayerId('mynameiszak', function (playerShots) {
    var html = [];
...
$(function() {
	...
	var button = '.dribbble-follow-button',
		label = $(button).text(),
		username = $('a'+button).attr('href').toLowerCase().replace('http://twitter.com/', ''),
	...
	$.getJSON('http://api.twitter.com/1.1/followers/ids.json?cursor=-1&screen_name='+username+'&count=5000', function(data) {
		$(button).wrap('<div class="dribbble-follow-button '+disableCount+'" />')
        .parent().html('<a class="label" href="http://twitter.com/'+username+'" target="_blank"><i></i>'+label+'</a><a class="count" href="http://twitter.com/'+username+'/followers" target="_blank"><i></i><u></u>'+data.followers_count+' followers</a>');
